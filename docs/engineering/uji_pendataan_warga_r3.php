...
// Draft dihapus SEBELUM akunnya: FK sf_penilaian_perumahan.user_id ON DELETE SET NULL, jadi menghapus
// akun saja meninggalkan draft yatim (user_id NULL) - 3 baris per jalan, ditemukan 26 Sep 2026.
// Rekomendasi & berkas ikut terhapus lewat FK CASCADE; profil warga lewat CASCADE dari akun.
function cleanup($db,$ids){foreach($ids as $id){$db->q('DELETE FROM sf_penilaian_perumahan WHERE user_id=?',[$id]);$db->q('DELETE FROM usr_users WHERE id=?',[$id]);}
...
/* Harness disusulkan 18 Sep 2026 ke wizard dua cabang (157e275 + 22c790f): pendapatan angka, DTKS tidak
   ditanyakan, dan data profil dasar wajib di langkah matriks. */
function matriks() {
  return ['matrix_current_housing_code'=>'house_owned','area_condition_code'=>'slum',
          'matrix_land_ownership_code'=>'land_none','matrix_environment_condition_code'=>'env_slum_uninhabitable',
          'matrix_occupation_finance_code'=>'work_stable_or_unstable_no_subsidy',
          'matrix_marital_family_code'=>'family_married',
          'phone'=>'081234567890','birth_date'=>'1980-01-01','gender_code'=>'male','marital_status_code'=>'married',
          'education_code'=>'senior_high','occupation_code'=>'trader','employment_stability_code'=>'permanent','monthly_income'=>'1200000'];
}
...
try {
  ...
  [$s]=$sa->call('warga/pendataan',['step'=>'housing_family_detail','assessment_id'=>$draft['id'],'lock_version'=>$draft['lock_version'],'direction'=>'next','family_card_number'=>'0000000000002002','full_name'=>'Warga Simulasi Parsial','phone'=>'080000000002','birth_date'=>'1990-12-31','address'=>'Alamat Sintetis','gender_code'=>'male','marital_status_code'=>'married','education_code'=>'senior_high','occupation_code'=>'trader','income_band_code'=>'2_2_2_6','self_help_capability_code'=>'capable','has_savings'=>'1','housing_status_code'=>'owned','land_title_code'=>'hm','has_other_land'=>'0','has_other_house'=>'0','house_area_m2'=>'36','occupant_count'=>'3','family_count'=>'1']); check(in_array($s,[302,303],true),'langkah data warga tersimpan');
