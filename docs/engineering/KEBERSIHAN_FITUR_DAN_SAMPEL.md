...
| Temuan | Perbaikan |
|---|---|
| 1 | Panel "Kredensial Demo" dicabut dari halaman login, modal login, dan halaman syarat pengembang (beserta CSS dan skripnya); dokumen akun login tidak lagi memuat kredensial. Alat `docs/engineering/bersihkan_data_sampel.php` menemukan akun ber-kata-sandi-bawaan dari daftar kata sandi umum (dry-run bawaan; `--ya` mengubah: admin diputar acak dan wajib ganti, akun lain dinonaktifkan, tidak dihapus). **Alat itu belum dijalankan di live**, lihat bagian 4. |
