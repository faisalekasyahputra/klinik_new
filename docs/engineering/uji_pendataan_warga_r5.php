...
$d=draft($db,$user); $san=['has_window'=>'1','has_ventilation'=>'1','water_source_code'=>'well','bathroom_usage_code'=>'own','latrine_type_code'=>'swan_neck','feces_disposal_code'=>'septic_tank','septic_distance_code'=>'gte_10','lighting_source_code'=>'pln','cooking_fuel_code'=>'electric_gas']; wajib(redirect_ok(post_step($owner,$d,'sanitation',$san)),'Simpan sanitasi');
...
$db->run("UPDATE sf_rekomendasi_penilaian SET eligibility_status='needs_data' WHERE assessment_id=? AND ruleset_version='".Warga_ruleset::VERSION."'",[$d['id']]); $blockedPage=$owner->get('warga/pendataan'); $blockedAction=strpos($blockedPage['body'],'Pengajuan belum dapat dikirim')!==FALSE && strpos($blockedPage['body'],'Lihat layanan lain')!==FALSE && strpos($blockedPage['body'],'Ajukan program yang dipilih')===FALSE;
...
cek((int)$db->scalar("SELECT COUNT(*) FROM sf_rekomendasi_penilaian WHERE assessment_id=? AND ruleset_version='".Warga_ruleset::VERSION."'",[$d['id']])===3,'Ganti ruleset aktif idempoten tanpa duplikat');
...
$before=(int)$db->scalar("SELECT COUNT(*) FROM sf_rekomendasi_penilaian WHERE assessment_id=? AND ruleset_version='".Warga_ruleset::VERSION."'",[$d['id']]);
...
cek(redirect_ok($r) && (int)$db->scalar("SELECT COUNT(*) FROM sf_rekomendasi_penilaian WHERE assessment_id=? AND ruleset_version='".Warga_ruleset::VERSION."'",[$d['id']])===$before,'Program hilang menggagalkan write secara atomik');
