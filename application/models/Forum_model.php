...
class Forum_model extends CI_Model {
    ...
    public function get_komentar_by_diskusi($id) {
        ...
        $flat = $this->db->get_where('tb_komentar', ['id_diskusi' => $id])->result_array();
    ...
    public function toggle_like($user_id, $target_type, $target_id) {
        $existing = $this->db->get_where('tb_forum_likes', [
            'user_id'     => $user_id,
            'target_type' => $target_type,
            'target_id'   => $target_id
        ])->row();

        $table = ($target_type === 'diskusi') ? 'tb_diskusi' : 'tb_komentar';
        $id_col = ($target_type === 'diskusi') ? 'id_diskusi' : 'id_komentar';

        if ($existing) {
            // Unlike
            $this->db->delete('tb_forum_likes', ['id' => $existing->id]);
    ...
    public function has_liked($user_id, $target_type, $target_id) {
        return $this->db->get_where('tb_forum_likes', [
            'user_id'     => $user_id,
            'target_type' => $target_type,
            'target_id'   => $target_id
        ])->num_rows() > 0;
    ...
    public function get_user_likes($user_id, $diskusi_id) {
        ...
        $komentar_ids = $this->db->select('id_komentar')
                                 ->get_where('tb_komentar', ['id_diskusi' => $diskusi_id, 'is_deleted' => 0])
                                 ->result();
