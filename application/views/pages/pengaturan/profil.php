...
<div class="relative z-10 space-y-4">
    ...
    <?php
    // Dulu seluruh halaman dikunci `max-w-2xl`, jadi di layar dashboard yang
    // lebar isinya berhenti di tengah dan separuh kanan kosong melompong —
    // itu keluhan "hanya tampak separo". Sekarang tiga kolom: formulir utama
    // mengambil dua, kolom kanan menampung yang jarang disentuh.
    ?>
    <div class="grid gap-4 lg:grid-cols-3">

        <!-- Informasi Profil -->
        <div class="<?= $kotak ?> lg:col-span-2">
            <h2 class="<?= $judul ?>">
                <i class="ph ph-identification-card text-brand-primary"></i> Informasi Profil
            </h2>

            <form action="<?= base_url('akun/update') ?>" method="POST" class="mt-4 space-y-4">
                ...
                <div class="border-t border-gray-100 pt-4 dark:border-white/5">
                    ...
                    <input type="password" name="current_password" autocomplete="current-password" placeholder="Password Anda saat ini (wajib untuk ganti password)" class="<?= $isian ?> mt-3">
                </div>
        ...
        <!-- Zona Berbahaya -->
        <div class="rounded-2xl border border-red-200 bg-red-50 p-5 dark:border-red-500/20 dark:bg-red-500/5">
            ...
            -->
            <p class="mt-3 text-xs text-gray-600 dark:text-brand-muted">Menghapus akun akan menghapus <b>akun dan akses masuk Anda</b>, serta dokumen SRP2 yang pernah Anda unggah. Diskusi dan komentar tidak dihapus, melainkan dianonimkan menjadi "Akun Dihapus" agar alur diskusi tidak rusak. Tindakan ini tidak bisa dibatalkan.</p>

            <p class="mt-2 text-xs text-gray-600 dark:text-brand-muted">Data layanan yang pernah Anda kirimkan — data pendataan perumahan, hasil penilaian, dan foto bukti — <b>tidak ikut terhapus saat ini</b> dan mengikuti kebijakan retensi data yang berlaku. Untuk meminta penghapusannya, hubungi admin.</p>

            <button type="button" onclick="openDeleteModal()" class="mt-4 inline-flex items-center gap-2 rounded-lg border border-red-200 bg-red-100 px-4 py-2 text-sm font-bold text-red-600 transition-colors hover:bg-red-200 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-400 dark:hover:bg-red-500/20">
