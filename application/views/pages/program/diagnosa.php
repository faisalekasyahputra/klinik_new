    ...
    <div class="text-center mb-10">
        ...
        <div class="mb-12 max-w-[80%] mx-auto">
            <div class="flex items-center justify-between relative">
                ...
                <div class="absolute left-0 top-1/2 -translate-y-1/2 h-0.5 bg-[color:var(--portal-brand)] rounded-full z-0 transition-all duration-500" :style="'width: ' + ((step - 1) / 2 * 100) + '%'"></div>

                <!-- Steps Indicators (3 Steps - Etalase digabung ke Langkah 1
                     sejak 17 Agt 2026, lihat komentar di badan Step 1) -->
                <template x-for="i in 3" :key="i">
                    <div class="relative z-10 flex flex-col items-center w-10 h-10">
                        ...
                        <span class="absolute top-12 text-xs font-medium whitespace-nowrap transition-colors duration-300" :class="step >= i ? 'text-[color:var(--portal-brand)] font-bold' : 'text-[color:var(--portal-text-muted)]'"
                              x-text="i === 1 ? 'Data & Program' : (i === 2 ? 'Identitas' : 'Konfirmasi')"></span>
                    </div>
                </template>
            </div>
        </div>
        ...
        <div id="form-diagnosa" class="w-full md:w-[80%] mx-auto border rounded-2xl p-5 sm:p-7 relative overflow-hidden scroll-mt-5">
            ...
            <form id="diagnosaForm" action="<?= base_url('Program/submit_antrean') ?>" method="POST">
                ...
                <div x-show="step === 1" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-8" x-transition:enter-end="opacity-100 translate-x-0">
                    ...
                    <div class="mt-6 flex justify-end">
                        <button type="button" @click="validateSurvey()" class="px-5 py-2.5 text-sm rounded-xl font-semibold bg-[color:var(--portal-brand)] text-[#0a1a1f] hover:brightness-95 transition-colors flex items-center gap-2" :disabled="!isSurveyComplete() || isLoading" :class="{'opacity-50 cursor-not-allowed': !isSurveyComplete() || isLoading}">
                            <span x-text="isLoading ? 'Menghitung...' : (eligiblePrograms.length ? 'Hitung Ulang' : 'Temukan Program')"></span>
                            <i class="fa-solid fa-spinner fa-spin" x-show="isLoading"></i>
                            <i class="fa-solid fa-microchip" x-show="!isLoading"></i>
                        </button>
                    </div>

                    <!-- Etalase Pilihan Program - permintaan user 17 Agt 2026:
                         "sebelum masuk langkah ke 2, hasil kelayakan sudah
                         muncul". Dulu ini "Step 2" tersendiri (baru terlihat
                         sesudah pindah layar); sekarang jadi bagian bawah
                         Langkah 1 yang sama, terbuka begitu eligiblePrograms
                         terisi - TANPA pindah step. Wizard jadi 3 langkah
                         total (Data & Program / Identitas / Konfirmasi),
                         bukan 4 - lihat penomoran ulang di bawah dan di
                         progress bar. Data survei di atas SENGAJA tetap
                         terlihat (bukan disembunyikan) supaya tombol "Hitung
                         Ulang" masuk akal kalau orang mau mengubah isian. -->
                    <div x-show="eligiblePrograms.length > 0" style="display: none;" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="mt-8 pt-6 border-t border-white/10">
                        ...
                        <div class="flex justify-end">
                            <button type="button" @click="step = 2" class="px-5 py-2.5 text-sm rounded-xl font-semibold bg-[color:var(--portal-brand)] text-[#0a1a1f] hover:brightness-95 transition-colors flex items-center gap-2" :disabled="!chosenProgram" :class="{'opacity-50 cursor-not-allowed': !chosenProgram}">
                                Lanjut Verifikasi Identitas <i class="fa-solid fa-arrow-right"></i>
                            </button>
                        </div>
                    </div>
                </div>

                <!-- Step 2: Verifikasi Identitas (NIK & SIMPERUM Fetch) -->
                <div x-show="step === 2" style="display: none;" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-8" x-transition:enter-end="opacity-100 translate-x-0">
                    ...
                    <p class="text-xs text-zinc-400 mb-4">Anda memenuhi kriteria awal untuk <strong class="text-white" x-text="chosenProgram?.title"></strong>. Verifikasi NIK untuk melanjutkan ke pengajuan.</p>
                    ...
                    <div class="mt-6 flex justify-between">
                        ...
                        <div class="flex gap-3">
                            ...
                            <button type="button" x-show="simperumData" @click="step = 3" class="px-5 py-2.5 text-sm rounded-xl font-semibold bg-[color:var(--portal-brand)] text-[#0a1a1f] hover:brightness-95 transition-colors flex items-center gap-2">
                                Lanjut ke Konfirmasi <i class="fa-solid fa-arrow-right"></i>
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div x-show="step === 3" style="display: none;" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-8" x-transition:enter-end="opacity-100 translate-x-0">
                    ...
                    <div class="flex justify-between">
                        <button type="button" @click="step = 1" class="px-5 py-2.5 text-sm rounded-xl font-semibold bg-white/5 text-white hover:bg-white/10 transition-colors">Ganti Program</button>
...
function wizardData() {
    return {
        ...
        errorMsg: '',
        simperumData: null,
        // Desil (kategori penghasilan hasil kalkulasi) - variabel SENDIRI,
        // bukan properti di dalam simperumData. Kelayakan dihitung di
        // Langkah 1 (Data Survei) SEBELUM identitas dicek di Langkah 2 -
        ...
        async fetchSimperum() {
            ...
            try {
                ...
                if(result.status === 'success') {
                    /* Susunan wizard 16 Agt 2026: identitas kini step TERAKHIR
                       sebelum konfirmasi, sesudah kelayakan & program sudah
                       dipilih (permintaan user - "isi data diri setelah
                       ketemu kelayakannya"). eligiblePrograms/chosenProgram
                       SUDAH terisi dari validateSurvey() di step sebelumnya -
                       menimpanya di sini (seperti versi lama) akan menghapus
                       pilihan program yang sudah dibuat orangnya. Prefill
                       survey dari data SIMPERUM juga dibuang - survei sudah
                       lewat dan tidak lagi ditampilkan di layar begitu
                       identitas dicek. */
            ...
            if(this.isSurveyComplete()) {
                ...
                try {
                    ...
                    if(result.status === 'success') {
                        ...
                        // Update the decile and available programs. `desil`
                        // sekarang variabel sendiri, TIDAK digantungkan ke
                        // simperumData - identitas belum tentu sudah dicek
                        ...
                        // TIDAK berpindah step (permintaan user 17 Agt 2026: hasil
                        // kelayakan muncul sebelum masuk langkah ke-2). Etalase
                        // program terbuka sendiri di layar yang sama begitu
                        // eligiblePrograms terisi - lihat x-show di badan Step 1.
                    } else {
