...
<section class="mx-auto max-w-4xl px-1 py-3 sm:px-3 sm:py-5 font-outfit" style="color:var(--portal-text)">
    ...
    <form id="form-pendataan-warga" action="<?= html_escape($action_url) ?>" method="post" enctype="multipart/form-data" class="scroll-mt-4 rounded-2xl border p-4 sm:p-6" style="background:var(--portal-bg-card);border-color:var(--portal-border)">
        ...
        <?php elseif ($step_slug === 'housing_family_detail'): ?>
            ...
            ?>
            <h2 class="text-lg font-black">Lengkapi Data SIMPERUM — Data Warga & Rumah</h2>
            <p class="mt-1 text-xs" style="color:var(--portal-text-muted)">Pilih <strong>Milik sendiri</strong> atau <strong>Bukan milik sendiri</strong>. Sistem akan menampilkan formulir lanjutan yang sesuai daftar UAT.</p>
            ...
            <div class="mt-3 grid gap-4 sm:grid-cols-2">
                ...
                <?php foreach ($housing as $key => [$label, $options]): ?><div <?= $key === 'land_title_code' ? 'data-uat-owned' : '' ?>><label for="<?= $key ?>" class="text-xs font-bold"><?= $label ?> <?= $badge($key) ?></label><select id="<?= $key ?>" name="<?= $key ?>" <?= $key === 'housing_status_code' ? 'required' : '' ?> aria-describedby="<?= $key ?>-error" aria-invalid="<?= $field_error($key) ? 'true' : 'false' ?>" class="mt-1 block w-full rounded-xl border px-3 py-2.5 text-sm" style="background:var(--portal-btn-bg);border-color:<?= $field_error($key) ? '#dc2626' : 'var(--portal-border)' ?>;color:var(--portal-text)"><option value="">Pilih <?= strtolower($label) ?></option><?php foreach ($options as $code => $label): ?><option value="<?= html_escape($code) ?>"<?= $selected($key, $code) ?>><?= html_escape($label) ?></option><?php endforeach; ?></select><p id="<?= $key ?>-error" class="mt-1 text-xs text-red-700"><?= html_escape($field_error($key)) ?></p></div><?php endforeach; ?>
                <?php foreach (['occupant_count' => 'Jumlah penghuni', 'family_count' => 'Jumlah keluarga', 'house_area_m2' => 'Luas rumah (m²)'] as $key => $label): ?><div <?= $key === 'house_area_m2' ? 'data-uat-owned' : '' ?>><label for="<?= $key ?>" class="text-xs font-bold"><?= $label ?> <?= $badge($key) ?></label><input id="<?= $key ?>" name="<?= $key ?>" type="number" min="<?= $key === 'house_area_m2' ? '0.01' : '1' ?>" step="<?= $key === 'house_area_m2' ? '0.01' : '1' ?>" <?= $key !== 'house_area_m2' ? 'required' : '' ?> value="<?= html_escape($value($key)) ?>" aria-describedby="<?= $key ?>-error" aria-invalid="<?= $field_error($key) ? 'true' : 'false' ?>" class="mt-1 block w-full rounded-xl border px-3 py-2.5 text-sm" style="background:var(--portal-btn-bg);border-color:<?= $field_error($key) ? '#dc2626' : 'var(--portal-border)' ?>;color:var(--portal-text)"><p id="<?= $key ?>-error" class="mt-1 text-xs text-red-700"><?= html_escape($field_error($key)) ?></p></div><?php endforeach; ?>
                <?php foreach (['has_other_land' => 'Memiliki tanah lain', 'has_other_house' => 'Memiliki rumah lain'] as $key => $label): ?><fieldset <?= $key === 'has_other_house' ? 'data-uat-owned' : '' ?>><legend class="text-xs font-bold"><?= $label ?> <?= $badge($key) ?></legend><div class="mt-2 flex gap-4 text-xs"><label><input type="radio" name="<?= $key ?>" value="1"<?= $checked($key, '1') ?>> Memiliki</label><label><input type="radio" name="<?= $key ?>" value="0"<?= $checked($key, '0') ?>> Tidak memiliki</label></div><p class="mt-1 text-xs text-red-700"><?= html_escape($field_error($key)) ?></p></fieldset><?php endforeach; ?>
                <div data-uat-not-owned><label for="assistance_source_code" class="text-xs font-bold">Sumber bantuan sebelumnya <span class="font-normal" style="color:var(--portal-text-muted)">(bila ada)</span></label><select id="assistance_source_code" name="assistance_source_code" class="mt-1 block w-full rounded-xl border px-3 py-2.5 text-sm" style="background:var(--portal-btn-bg);border-color:var(--portal-border);color:var(--portal-text)"><option value="">Belum pernah/tidak diisi</option><?php foreach (['apbn_bsps' => 'APBN/BSPS (data SIMPERUM)', 'apbn' => 'APBN', 'apbd_prov' => 'APBD Provinsi', 'apbd_kab' => 'APBD Kab/Kota', 'csr' => 'CSR', 'village_fund' => 'Dana Desa', 'bsps_kl' => 'BSPS-KL', 'bankab' => 'BANKAB', 'baznas' => 'BAZNAS', 'other' => 'Sumber lainnya'] as $code => $label): ?><option value="<?= $code ?>"<?= $selected('assistance_source_code', $code) ?>><?= $label ?></option><?php endforeach; ?></select></div>
                <div data-uat-not-owned><label for="assistance_year" class="text-xs font-bold">Tahun bantuan <span class="font-normal" style="color:var(--portal-text-muted)">(bila ada)</span></label><input id="assistance_year" name="assistance_year" type="number" min="1900" max="<?= date('Y') ?>" step="1" value="<?= html_escape($value('assistance_year')) ?>" class="mt-1 block w-full rounded-xl border px-3 py-2.5 text-sm" style="background:var(--portal-btn-bg);border-color:var(--portal-border);color:var(--portal-text)"></div>
            </div>
            <p id="uat-branch-help" class="mt-5 rounded-xl p-3 text-xs" style="background:rgba(14,165,233,.09);color:#075985"></p>
<script>document.addEventListener('DOMContentLoaded',function(){var s=document.getElementById('housing_status_code');if(!s)return;function show(){var owned=s.value==='owned';document.querySelectorAll('[data-uat-owned]').forEach(function(e){e.hidden=!owned});document.querySelectorAll('[data-uat-not-owned]').forEach(function(e){e.hidden=owned||!s.value});var h=document.getElementById('uat-branch-help');h.textContent=!s.value?'Pilih status tempat tinggal untuk membuka cabang formulir.':owned?'Cabang Milik Sendiri: selanjutnya isi kondisi bangunan, sanitasi, koordinat, dan foto rumah.':'Cabang Bukan Milik Sendiri: selanjutnya isi alamat, legalitas, asal, hubungan pemilik, ukuran lahan, Foto Lahan, dan Bukti Pindah Tangan.';}s.addEventListener('change',show);show();});</script>
