                    ...
                    <tbody data-table-body>
                        <?php foreach ($daftar_pengembang as $i => $row): ?>
                            <tr data-table-row class="border-t transition-colors hover:bg-[#00a3b5]/[.04]" style="border-color:var(--portal-border)">
                                ...
                                <?php $wilayah = $nama_kabupaten[(string) ($row->kabupaten_id ?? '')] ?? 'Belum tercatat'; $akhir = trim((string) ($row->sertifikat_berakhir ?? '')); $aktif = in_array((string) $row->status_sertifikasi, ['Diterima', 'bersertifikat'], TRUE) && $akhir !== '' && strtotime($akhir . ' 23:59:59') >= time(); /* Tanpa tanggal akhir = Tidak berlaku (keputusan 23 Sep 2026, UAT #12/#13) */ $periode = trim((string) ($row->sertifikat_terbit ?? '')) !== '' ? date('d M Y', strtotime($row->sertifikat_terbit)) . ' - ' . ($akhir !== '' ? date('d M Y', strtotime($akhir)) : 'Belum ditentukan') : ($akhir !== '' ? 's.d. ' . date('d M Y', strtotime($akhir)) : 'Belum ditentukan'); ?>
