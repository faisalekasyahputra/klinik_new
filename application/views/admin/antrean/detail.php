...
<div class="space-y-5">
    ...
    <section class="rounded-2xl border border-gray-200 p-4 dark:border-white/10"><h2 class="font-black text-gray-900 dark:text-white">Bukti privat</h2><div class="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3"><?php foreach ($evidence as $file): ?><a href="<?= base_url(($evidence_url ?? 'Admin/evidence') . '/' . (int) ($queue['id'] ?? 0) . '/' . rawurlencode($file['file_kind'])) ?>" data-file-view data-file-title="<?= $e($file['file_kind']) ?>" class="rounded-xl border border-gray-200 p-3 text-sm font-bold text-brand-primary dark:border-white/10">Lihat <?= $e($file['file_kind']) ?><span class="block text-xs font-normal text-gray-500"><?= number_format(((int) ($file['size_bytes'] ?? 0)) / 1024, 0, ',', '.') ?> KB</span></a><?php endforeach; ?><?php if (!$evidence): ?><p class="text-sm text-gray-500">Belum ada bukti.</p><?php endif; ?></div></section>
