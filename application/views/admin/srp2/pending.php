    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-white/5 text-gray-700 dark:text-gray-300">
                <?php if (empty($rows)): ?>
                <tr>
                    <td colspan="5" class="px-6 py-12 text-center text-gray-500 dark:text-brand-muted">Tidak ada pengajuan<?= $status_filter === 'semua' ? '.' : ' berstatus ' . html_escape($label_status[$status_filter] ?? $status_filter) . '.' ?></td>
                </tr>
